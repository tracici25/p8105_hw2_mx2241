...
Import NYC Transit Dataset

Import the data:
```{r}
transit_df = 
  read_csv("./data/NYC_Transit_Subway_Entrance_And_Exit_Data.csv") %>% 
  janitor::clean_names() #clean names for column titles
```
...
```{r}
transit_df = 
  select(transit_df, line, station_name, station_latitude, station_longitude, route1:route11, entry, vending, entrance_type, ada) %>% #select the variables of interest
  mutate_at(vars(route8:route11), as.character) %>% #change class from double to character
  mutate(
