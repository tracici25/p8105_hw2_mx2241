...
`distinct` function was used to calculate the number of distinct stations. It was found that there are `r nrow(distinct(transit_df, line, station_name))` distinct stations. Out of these station, there are `r transit_df %>% filter(ada == TRUE) %>% distinct(line, station_name) %>% nrow()` stations that are ADA compliant. The proportion of station entrances/exits without vending allow entrance is `r (filter(transit_df, entry == "TRUE", vending == "NO") %>% nrow / filter(transit_df, vending == "NO") %>% nrow)*100`%.


To reformat the dataset further more
```{r}
